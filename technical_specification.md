...
## Основной дорожный граф

* Ways — OSM way + привязка к городу (`id_city`). Геометрию не содержит; тип/имя/oneway лежат в `WayProperties`.
* Points — все узлы OSM, которые участвуют в `Ways`; lon/lat для построения геометрии.
* Edges — ориентированные сегменты между соседними узлами way. Для двусторонних дорог добавляется пара рёбер, для oneway — одно.
* Связи: `Edges.id_way → Ways.id`, `Edges.id_src/id_dist → Points.id`, фильтрация по городу через `Ways.id_city`.

## Access-граф (подъезды/пешеходные связи)

* AccessNodes — перекрёстки жилых/служебных дорог и центроиды зданий; содержат `node_type`, `source_type/id`, координаты, имя/теги.
* AccessEdges — связи между AccessNodes; `is_building_link`=true для коротких подъездов «здание ↔ дорога», иначе дворовые/пешеходные сегменты.
* Зачем отдельно: иные атрибуты и назначение; не засоряем основной граф подъездами; можно перестраивать независимо.

## Источник данных и пайплайн OSM

1. Osmosis загружает PBF в служебные таблицы схемы osmosis (`ways`, `nodes`, `way_nodes`, `way_tags` и др.) в той же БД. Эти таблицы используются как временный буфер и могут перезаписываться при новой загрузке.
2. Копирование в наши таблицы:

- `Ways` ← `ways` (id way, id города);
- `Points` ← `nodes`, которые участвуют в `way_nodes` для загруженных `Ways` (берём lon/lat из `geom`);
- `Edges` — по последовательности `way_nodes` (с учётом oneway: одно или два направления);
- `WayProperties` — теги из `way_tags` (highway, name, oneway и др.) в виде `(id_way, id_property, value)`.

3. Access-граф строится отдельным шагом напрямую из PBF через `build_access_graph`: парсинг жилых/служебных дорог и зданий, поиск перекрёстков, центроидов зданий, привязка точки к ближайшей дороге, если он в пределах заданного расстояния(в нашем access-графе мы привязываем центроид здания к ближайшему узлу дороги, если ближе, чем `snap_distance_m`), генерация `AccessNodes `и `AccessEdges `с флагом `is_building_link` для подъездов.

## Переносимые теги OSM

| Тег OSM          | Куда пишем                      | Назначение                         |
| ------------------- | ---------------------------------------- | -------------------------------------------- |
| `highway`         | `WayProperties`                        | Тип дороги (primary, residential)   |
| `name`            | `WayProperties`, `AccessEdges`       | Название улицы/сегмента |
| `oneway`          | `WayProperties`                        | Направленность улицы      |
| `service`         | `WayProperties`, access-фильтр   | driveway/parking_aisle для access         |
| `building`        | Входной тег для AccessNodes | Выделение зданий              |
| `layer`/`level` | `WayProperties` (если есть)    | Доп. атрибуты                     |

## Правила построения Edges

* Источник: последовательности `way_nodes` для каждого `way` данного города.
* Для `oneway=yes` создаётся одно направленное ребро id_src → id_dist.
* Для двусторонних улиц вставляются два направления (вперёд и назад по sequence_id).
* Используются только узлы из `Points`; совпадения src=dst не вставляются.

## Правила построения access-графа

* Отбор дорог: `highway` в {service, residential, living_street, unclassified, track, road} + `service=driveway/parking_aisle`.
* Отбор зданий: все `building=*`, центроид полигона становится узлом `node_type=building`.
* Перекрёстки малых дорог становятся узлами `node_type=intersection`.
* Привязка: если здание ближе к дороге, чем `snap_distance_m` (по умолчанию 30 м), добавляется ребро `is_building_link=true` к ближайшему узлу дороги.
* Остальные сегменты между узлами access строятся с `is_building_link=false`, `road_type` берётся из входного `highway`/`service`.

## Связь с фронтом и CSV

* Основной граф: `points_csv` (id, lon, lat), `edges_csv` (id, id_way, source, target, name) → синий слой (узлы/линии улиц).
* Access-граф: `access_nodes_csv` (id, node_type, lon, lat, source_type/id, name), `access_edges_csv` (id, source, target, road_type, length_m, is_building_link, name) → зелёные (building) и оранжевые (intersection) точки и линии подъездов/пешеходки.
* Экспорт: `combined_nodes_csv`/`combined_edges_csv` (слои base/access), `metrics_csv` (degree, betweenness и т.д.).
