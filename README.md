...
Проект для анализа топологии городов и городских сетей.

# Цели проекта 
* Определять среднее число связей (пересечений улиц) для каждой улицы города, как по городу в целом, так и по отдельным районам.
* Дать исследователям инструмент для выявления самоорганизующихся городских пространств и сравнения с регулярным (системным) планированием.
* Визуализировать топологию улично-дорожной сети в виде графа и вычислять ключевые параметры данной топологии.
# Задачи проекта
* Разработать веб-приложение для исследования и анализа улично-дорожной сети любого города.
* Реализовать загрузку и обработку карт города (OSM) с построением графа улиц и вычислением числа связей.
* Представлять и визуализировать топологию улиц (узлы, ребра, районы) в графовом виде, с фильтрацией по районам.
* Поддержать анализ по районам: считать и сравнивать метрики связности/структуры для отдельных пространств.
* Обеспечить фронтенд (JS/TS, современный фреймворк) с рендером графа/карты; настроить сборку и статический хостинг.
* Обеспечить бэкенд API для загрузки данных, вычислений и выдачи результатов (метрики, граф, экспорт).
* Настроить инфраструктуру: логирование, конфиг БД, кеш/экспорт результатов, базовые автотесты.

## 📋 Содержание
- [Предварительные действия](#предварительные-действия)
- [Запуск приложения](#запуск-приложения)
- [Выключение сервера](#выключение-сервера)
- [Тесты](#тесты)
- [Анализ графов](#анализ-графов)
- [Граф придомовых проездов](#граф-придомовых-проездов)
- [Экспорт CSV](#экспорт-csv)
- [Утилиты в каталоге `tools`](#утилиты-в-каталоге-tools)

## <a id="предварительные-действия">Предварительные действия</a>

1. Скачайте PBF-файлы исследуемых городов и поместите их в директорию `/cities_pbf/` в корне проекта (создайте её, если отсутствует).
2. Названия файлов должны иметь вид: `{Название города с большой буквы}.pbf`. Пример: `Москва.pbf`.
3. Для скачивания готовых файлов можно воспользоваться [https://extract.bbbike.org/](https://extract.bbbike.org/) или скриптом `tools/osm_fetch.py` (функция `download_city`).

## <a id="запуск-приложения">Запуск приложения</a>
...
- Сгенерировать HTML-отчёт о тестах и покрытии:
 ...
 ## <a id="анализ-графов">Анализ графов</a>
...
На вход ноутбуки принимают JSON-файл, которые возвращают в качестве ответа методы `/api/city/graph/region/` и `/api/city/graph/bbox/`.
 ...
 ## <a id="граф-придомовых-проездов">Граф придомовых проездов</a>
...
Фронтенд (виджет «Дороги») отображает новый слой поверх основного графа: здания показаны зелёными маркерами, внутренние узлы — оранжевыми, подъездные рёбра — выделенными линиями.
 ...
 ## <a id="экспорт-csv">Экспорт CSV</a>
...
Файл можно использовать для дальнейшего анализа или сторонних визуализаций без необходимости ручного склеивания слоёв.
 ...
 ## <a id="утилиты-в-каталоге-tools">Утилиты в каталоге `tools`</a>
